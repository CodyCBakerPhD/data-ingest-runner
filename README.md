...
2. On the runner host, install and configure:
   ...
   - The [`dandi` CLI](https://github.com/dandi/dandi-cli). Set the `EMBER_DANDI_API_KEY` **repository secret** (Settings -> Secrets and variables -> Actions -> Secrets) to a DANDI API key for the `emberarchive` instance; the workflow exposes it to `dispatch.py` as `DANDI_API_KEY`, which every `dandi`/`docker run` subprocess inherits, and which is also forwarded (by name only, never as a literal value) into a project's `container_image` when one's set, for labs whose conversion step itself needs DANDI access. Verify against your installed `dandi` CLI version whether a plain `DANDI_API_KEY` env var authenticates a *named* instance (`-i emberarchive`) the way you expect — if it doesn't, `dandi login -i emberarchive` (run once interactively on the runner, stored via the OS keyring) is the fallback.
